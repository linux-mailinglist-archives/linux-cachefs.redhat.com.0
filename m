...
Received: from us-smtp-delivery-124.mimecast.com (us-smtp-delivery-124.mimecast.com [170.10.133.124])
	by mail.lfdr.de (Postfix) with ESMTPS id DFA054F6E37
	for <lists+linux-cachefs@lfdr.de>; Thu,  7 Apr 2022 01:03:38 +0200 (CEST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=redhat.com;
	s=mimecast20190719; t=1649286217;
	...
	bh=1LlrtAGVuRkFTFHcwA5A0YnQbWaryJyXuycUo3HpgP4=;
	b=e5mkkdpeYVsYkw0BQzLCRVJrDDBfHZdy23gQ7v2y6XBqQypswNKQfOlnwFTOmxpcc8ZBdy
	ybFyA5J0XI7LZWPpKcabHj5whV9sytlVr0oF/x5CHkbxf7Vq73lvCz1d/npE16qso+KWNi
	QQ5fqwKBt5ZYp1+iOgI/Mmzrv+dTcfI=
Received: from mimecast-mx02.redhat.com (mx3-rdu2.redhat.com
 [66.187.233.73]) by relay.mimecast.com with ESMTP with STARTTLS
 (version=TLSv1.2, cipher=TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384) id
 us-mta-158-eq1DEh-PPluH9ZCELx2BmA-1; Wed, 06 Apr 2022 19:03:34 -0400
X-MC-Unique: eq1DEh-PPluH9ZCELx2BmA-1
Received: from smtp.corp.redhat.com (int-mx01.intmail.prod.int.rdu2.redhat.com [10.11.54.1])
	(using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
	(No client certificate requested)
	by mimecast-mx02.redhat.com (Postfix) with ESMTPS id 47BC73C11A23;
	Wed,  6 Apr 2022 23:03:34 +0000 (UTC)
Received: from mm-prod-listman-01.mail-001.prod.us-east-1.aws.redhat.com (mm-prod-listman-01.mail-001.prod.us-east-1.aws.redhat.com [10.30.29.100])
	by smtp.corp.redhat.com (Postfix) with ESMTP id 3460640CF918;
	Wed,  6 Apr 2022 23:03:34 +0000 (UTC)
Received: from mm-prod-listman-01.mail-001.prod.us-east-1.aws.redhat.com (localhost [IPv6:::1])
	by mm-prod-listman-01.mail-001.prod.us-east-1.aws.redhat.com (Postfix) with ESMTP id E3FD2194036B;
	Wed,  6 Apr 2022 23:03:33 +0000 (UTC)
X-Original-To: linux-cachefs@listman.corp.redhat.com
Delivered-To: linux-cachefs@listman.corp.redhat.com
Received: from smtp.corp.redhat.com (int-mx05.intmail.prod.int.rdu2.redhat.com
 [10.11.54.5])
 by mm-prod-listman-01.mail-001.prod.us-east-1.aws.redhat.com (Postfix) with
 ESMTP id 23E3C1949763 for <linux-cachefs@listman.corp.redhat.com>;
 Wed,  6 Apr 2022 23:03:33 +0000 (UTC)
Received: by smtp.corp.redhat.com (Postfix)
 id 1001476C4; Wed,  6 Apr 2022 23:03:33 +0000 (UTC)
Delivered-To: linux-cachefs@redhat.com
Received: from warthog.procyon.org.uk (unknown [10.33.37.45])
 by smtp.corp.redhat.com (Postfix) with ESMTP id 44BC47ADF;
 Wed,  6 Apr 2022 23:03:22 +0000 (UTC)
...
Date: Thu, 07 Apr 2022 00:03:21 +0100
Message-ID: <164928620163.457102.11602306234438271112.stgit@warthog.procyon.org.uk>
...
Subject: [Linux-cachefs] [PATCH 04/14] cifs: Change the I/O paths to use an
 iterator rather than a page list
...
---
 ...
 9 files changed, 770 insertions(+), 811 deletions(-)
...
index d511a78383c3..1063ec88a118 100644
--- a/fs/cifs/file.c
+++ b/fs/cifs/file.c
...
+
 ...
 {
@@ -4399,19 +4513,66 @@ cifs_readpages_copy_into_pages(struct TCP_Server_Info *server,
 {
...
@@ -4420,7 +4581,6 @@ static void cifs_readahead(struct readahead_control *ractl)
 	else
 		pid = current->tgid;
...
@@ -4430,7 +4590,7 @@ static void cifs_readahead(struct readahead_control *ractl)
...
+		unsigned int i, rsize;
 		...
 		struct cifs_readdata *rdata;
 		struct cifs_credits credits_on_stack;
@@ -4508,33 +4668,28 @@ static void cifs_readahead(struct readahead_control *ractl)
 			break;
 		}
...
@@ -4545,18 +4700,16 @@ static void cifs_readahead(struct readahead_control *ractl)
...
index db23f5b404ba..b0485706e07c 100644
--- a/fs/cifs/smb2ops.c
+++ b/fs/cifs/smb2ops.c
@@ -4452,15 +4452,30 @@ fill_transform_hdr(struct smb2_transform_hdr *tr_hdr, unsigned int orig_len,
...
@@ -4472,43 +4487,46 @@ static inline void smb2_sg_set_buf(struct scatterlist *sg, const void *buf,
...
+	return priv.sg;
 ...
 static int
@@ -4645,18 +4663,30 @@ crypt_message(struct TCP_Server_Info *server, int num_rqst,
 	return rc;
 }
...
@@ -4747,12 +4778,12 @@ smb3_is_transform_hdr(void *buf)
...
+	size_t iter_size = 0;
 	...
 	iov[0].iov_base = buf;
@@ -4762,10 +4793,10 @@ decrypt_raw_data(struct TCP_Server_Info *server, char *buf,
...
@@ -4776,73 +4807,37 @@ decrypt_raw_data(struct TCP_Server_Info *server, char *buf,
...
@@ -4851,9 +4846,6 @@ handle_read_data(struct TCP_Server_Info *server, struct mid_q_entry *mid,
...
@@ -4942,7 +4934,7 @@ handle_read_data(struct TCP_Server_Info *server, struct mid_q_entry *mid,
 			return 0;
 		}
...
+		if (data_len > pages_len - pad_len) {
 			...
 			rdata->result = -EIO;
 			if (is_offloaded)
@@ -4952,8 +4944,9 @@ handle_read_data(struct TCP_Server_Info *server, struct mid_q_entry *mid,
 			return 0;
 		}
...
@@ -4961,14 +4954,16 @@ handle_read_data(struct TCP_Server_Info *server, struct mid_q_entry *mid,
...
@@ -4980,13 +4975,6 @@ handle_read_data(struct TCP_Server_Info *server, struct mid_q_entry *mid,
 		return 0;
 	}
...
@@ -4997,9 +4985,8 @@ handle_read_data(struct TCP_Server_Info *server, struct mid_q_entry *mid,
...
@@ -5008,11 +4995,13 @@ static void smb2_decrypt_offload(struct work_struct *work)
 {
...
+			      &iter, true);
 	if (rc) {
 		...
 		goto free_pages;
@@ -5026,7 +5015,7 @@ static void smb2_decrypt_offload(struct work_struct *work)
 		mid->decrypted = true;
...
@@ -5058,10 +5047,7 @@ static void smb2_decrypt_offload(struct work_struct *work)
 	}
...
+	iov_iter_truncate(&iter, dw->len);
 	...
 	if (rc)
@@ -5124,39 +5129,28 @@ receive_encrypted_read(struct TCP_Server_Info *server, struct mid_q_entry **mid,
...
@@ -5166,9 +5160,9 @@ receive_encrypted_read(struct TCP_Server_Info *server, struct mid_q_entry **mid,
 	}
...
@@ -5206,7 +5200,7 @@ receive_encrypted_standard(struct TCP_Server_Info *server,
 	server->total_read += length;
...
@@ -5305,7 +5299,7 @@ smb3_handle_read_data(struct TCP_Server_Info *server, struct mid_q_entry *mid)
...
index eeb1a699bd6f..c74aa12ecc7c 100644
